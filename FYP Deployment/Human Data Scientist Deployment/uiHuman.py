...
def get_shap():
    ...
    if response.status_code == 200:
        shap_html = response.text
        # shap_html_with_background = """
        #     <!DOCTYPE html>
        #     <html>
        #     <head>
        #         <style>
        ...
        #st.subheader("SHAP Force Plot for the Entered Data")
...
def get_info_page():
    ...
    with st.form("my_form"):
        ...
        # Question 6       (Feature 6)
        instal_plans_vals = ['', 'A141', 'A142', 'A143']
        instal_plans_desc = ['', 'Bank', 'Store', 'None']
        instal_plans = st.selectbox('6. f) Other Instalment Plans', instal_plans_desc, format_func=lambda x: 'Select an option' if x == '' else x)
        instal_plans_index = instal_plans_vals[instal_plans_desc.index(instal_plans)]
        st.session_state.answers[5] = None if instal_plans_index == '' else instal_plans_index
        update_response(5)
            
    
        # Question 7        (Feature 7)
        housing_vals = ['', 'A151', 'A152', 'A153']
        housing_desc = ['', 'Rent', 'Own', 'For free']
        housing = st.selectbox('7. g) Housing', housing_desc,  format_func=lambda x: 'Select an option' if x == '' else x)
        housing_index = housing_vals[housing_desc.index(housing)]
        st.session_state.answers[6] = None if housing_index == '' else housing_index
        update_response(6)
        
        
        # Question 8         (Feature 8)
        telephone_vals = ['', 'A191', 'A192']
        telephone_desc = ['', 'None', 'Yes']
        telephone = st.selectbox('8. h) Has Telephone?', telephone_desc,  format_func=lambda x: 'Select an option' if x == '' else x)
        telephone_index = telephone_vals[telephone_desc.index(telephone)]
        st.session_state.answers[7] = None if telephone_index == '' else telephone_index
        update_response(7)
        
        
        # Question 9          (Feature 9)
        duration_vals = ["", "Short term", "Medium term", "Long term"]
        duration = st.selectbox('9. i) Duration', duration_vals,  format_func=lambda x: 'Select an option' if x == '' else x)
        duration_index = duration
        st.session_state.answers[8] = None if duration_index == '' else duration_index
        update_response(8)
        
        
        # Question 10         (Feature 10)
        ...
        purpose = st.selectbox('10. j) Purpose', purpose_desc,  format_func=lambda x: 'Select an option' if x == '' else x)
        purpose_index = purpose_vals[purpose_desc.index(purpose)]
        st.session_state.answers[9] = None if purpose_index == '' else purpose_index
        update_response(9)
        
        
        # Question 11         (Feature 11)
        properties_vals = ["", "A121",        "A122",                                              "A123",         "A124"]
        properties_desc = ["", "Real estate", "Building society savings agreement/life insurance", "Car or other", "Unknown / no property"]
        properties = st.selectbox('11. k) Property', properties_desc, format_func=lambda x: 'Select an option' if x == '' else x)
        properties_index = properties_vals[properties_desc.index(properties)]
        st.session_state.answers[10] = None if properties_index == '' else properties_index
        update_response(10) 
        
        
        # Question 12          (Feature 12)
        present_residence_duration = st.number_input('12. l) Present Residence Duration',  value=None, min_value=None, max_value=None, step=None, placeholder="Enter value")
        st.session_state.answers[11] = None if present_residence_duration is None else present_residence_duration
        update_response(11)
        
        
        # Question 13          (Feature 13)
            ...
            if agree:
                if check_response() == True:
                    ...
                    input_data = {
                        ...
                        "other_installment_plans": st.session_state.answers[5],
                        "housing": st.session_state.answers[6],
                        "telephone": st.session_state.answers[7],
                        "duration_category": st.session_state.answers[8],
                        "purpose": st.session_state.answers[9],
                        "property": st.session_state.answers[10],
                        "present_residence_duration": st.session_state.answers[11],
                    ...
                    if status == 200:
                        st.markdown("---")
                        predicted_class = prediction['prediction'][0][0]
                        ...
                        if predicted_class == 1:
                            st.markdown(f"""
                                    <div class="outcome">
                                        <h2>Outcome</h2>
                                        <b><span class="msg" style="color: red">Prediction      : {message}</span></b></br>
                                        <b><span class="msg" style="color: red">Suggested Action: Do not approve loan</span></b>
                                    </div>
                                    """, unsafe_allow_html = True
                            )
                        else:
                            st.markdown(f"""
                                    <div class="outcome">
                                        <h2>Outcome</h2>
                                        <b><span class="msg" style="color: green">Prediction      : {message}</span></b></br>
                                        <b><span class="msg" style="color: green">Suggested Action: Approve loan</span></b>
                                    </div>
                                    """, unsafe_allow_html = True
                            )
